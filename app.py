...
with st.sidebar:
    ...
    st.divider()
    category_placeholder = st.empty() # 카테고리 필터 공간
...
# [수정] 트위터 주급 맵일 때는 사이드바에 카테고리 버튼을 표시하지 않음 (본문에 내장됨)
...
if menu == "트위터 팔로워 맵":
    ...
    if not df.empty:
        ...
        if not display_df.empty:
            display_df['chart_label'] = display_df.apply(lambda x: f"{str(x['name'])}<br><span style='font-size:0.7em; font-weight:normal;'>@{str(x['handle'])}</span>", axis=1)
            display_df['log_followers'] = np.log10(display_df['followers'].replace(0, 1))
            fig = px.treemap(
                display_df, path=['category', 'chart_label'], values='followers', color='log_followers', custom_data=['name'], 
            ...
            fig.update_traces(
                ...
                textfont=dict(size=20, family="sans-serif", color="white"), textposition="middle center",
                marker=dict(line=dict(width=3, color='#000000')), root_color="#000000",
            ...
            fig.update_layout(margin=dict(t=0, l=0, r=0, b=0), paper_bgcolor='#000000', plot_bgcolor='#000000', height=600, font=dict(family="sans-serif"), coloraxis_showscale=False, hoverlabel=dict(bgcolor="#1C1F26", bordercolor="#10B981", font=dict(size=18, color="white"), namelength=-1))
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
            st.write("")
            
            col_head, col_toggle = st.columns([1, 0.3])
            with col_head: st.subheader("🏆 팔로워 순위 (Leaderboard)")
            with col_toggle: expand_view = st.toggle("전체 펼치기", value=False)
            ...
            for index, row in ranking_df.iterrows():
                ...
                bio_content = clean_str(row.get('bio', '')) if 'bio' in row else "소개글이 없습니다."
                ...
                <details {'open' if expand_view else ''}>
                    <summary>
                        <div class="ranking-row">
                            ...
                            <div class="rank-info"><div class="rank-name">{row['name']}</div><div class="rank-handle">@{row['handle']}</div></div>
                            <div class="rank-extra">{interest_html}{note_html}</div>
                            <div class="rank-stats-group">
                                <div class="rank-category">{row['category']}</div><div class="rank-share">{share_pct:.1f}%</div><div class="rank-followers">{int(row['followers']):,}</div>
                            </div>
                        </div>
                    </summary>
                    <div class="bio-box"><div class="bio-header">📝 PROFILE BIO</div><div class="bio-content">{bio_content}</div><a href="https://twitter.com/{row['handle']}" target="_blank" class="bio-link-btn">Visit Profile ↗</a></div>
                </details>
                """
            with st.container(height=600 if not expand_view else None): st.markdown(list_html, unsafe_allow_html=True)
...
# ==========================================
    ...
    # [수정] 인자를 2개만 전달 (conn, df) -> selected_category, merge_categories는 내부 처리
