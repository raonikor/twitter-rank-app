...
def get_data():
    try:
        ...
        if df is not None and not df.empty:
            ...
            # 핸들 (무조건 문자열로)
            df['handle'] = df['handle'].astype(str).replace('nan', '')
            
            # 이름 (없으면 핸들로 채우고, 무조건 문자열로)
            if 'name' not in df.columns:
                df['name'] = df['handle'] 
            else:
                df['name'] = df['name'].fillna(df['handle']).astype(str).replace('nan', '')
...
if not df.empty:
    ...
    if selected_category == "전체보기":
        display_df = df[df['followers'] > 0].copy() # .copy()로 경고 방지
    else:
        display_df = df[(df['category'] == selected_category) & (df['followers'] > 0)].copy()
    ...
    if not display_df.empty:
        # [에러 수정 핵심] 차트를 그리기 전에 데이터를 강제로 문자열(String)로 변환
        display_df['name'] = display_df['name'].astype(str)
        display_df['handle'] = display_df['handle'].astype(str)
