...
def render_payout_page(conn, follower_df):
    ...
    if not payout_df.empty:
        ...
        with col1: 
            st.markdown(f"""<div class="metric-card"><div class="metric-label">총 지급액 (Total Payout)</div><div class="metric-value">${total_payout:,.0f}</div></div>""", unsafe_allow_html=True)
        with col2: 
            st.markdown(f"""<div class="metric-card"><div class="metric-label">주급 1위 (Top Earner)</div><div class="metric-value">{top_earner['name']} (${top_earner['payout_amount']:,.0f})</div></div>""", unsafe_allow_html=True)
        ...
        fig = px.treemap(
            display_df, path=path_list, values='payout_amount', color='payout_amount', custom_data=['name', 'handle'],
        ...
        fig.update_layout(margin=dict(t=0, l=0, r=0, b=0), paper_bgcolor='#000000', plot_bgcolor='#000000', height=600, font=dict(family="sans-serif"), coloraxis_showscale=False)
        ...
        with col_head: st.subheader("📋 주급 랭킹 (Payout Ranking)")
        with col_toggle: expand_view = st.toggle("전체 펼치기", value=False, key="payout_ranking_toggle")
        ...
        for index, row in ranking_df.iterrows():
            ...
            follower_text = f"{int(row['followers']):,}" if 'followers' in row else "0"
            ...
            <details {'open' if expand_view else ''}>
                ...
                <div class="bio-box"><div class="bio-header">💰 PAYOUT INFO</div><div class="bio-content">{bio_content}</div><a href="https://twitter.com/{row['handle']}" target="_blank" class="bio-link-btn">Visit Profile ↗</a></div>
            </details>
            """
